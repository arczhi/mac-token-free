...
这个项目的目标很直接：帮助 Mac 用户在本地实现接近“token 自由”的编码 agent 体验。
...
Decode 侧是流畅的：MTP 明确启用，多数轮 decode wall time 在 1 到 4 秒。真正的等待主要在 prefill/调度/cache 侧，尤其 prompt 超过 35k 后更明显。
...
1. 安装 oMLX。
   ...
   推荐使用 oMLX 官方 DMG 或 Homebrew。确保 `omlx-cli` 可用，脚本会按这些路径自动查找：
...
不建议一上来把完整仓库、超长日志、巨量工具结果全塞进上下文。长上下文本身不是免费的：超过 35k tokens 后，decode 仍然快，但 prefill 等待会明显上来。
